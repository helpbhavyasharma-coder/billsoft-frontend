...
import { ArrowLeft, Plus, Trash2, X, Phone, MapPin, FileText, IndianRupee } from "lucide-react";
...
export default function PartyLedger() {
  ...
  const openGeneralPayModal = () => {
    const unpaidInvoices = (data?.invoices || []).filter(i => i.payment_status !== "paid");
    const firstUnpaid = unpaidInvoices[0];
    setSelectedInvoice(null);
    setPayForm({
      invoice_id: firstUnpaid ? firstUnpaid.id : "",
      amount: "",
      payment_date: format(new Date(), "yyyy-MM-dd"),
      payment_mode: "cash",
      reference_no: "",
  ...
  const handlePayment = async (e) => {
    e.preventDefault();
    if (!payForm.invoice_id) { toast.error("Select an invoice."); return; }
    if (!payForm.amount || parseFloat(payForm.amount) <= 0) { toast.error("Enter valid amount."); return; }
    setPaying(true);
    try {
      const { data: res } = await api.post("/payments", { ...payForm, invoice_id: parseInt(payForm.invoice_id) });
      if (res.success) {
  ...
  const { party, invoices, payments, summary } = data;
      ...
      {parseFloat(summary.outstanding) > 0 && (
        <div className="rounded-2xl p-4 flex items-center justify-between flex-wrap gap-3"
          ...
          <div>
            ...
            <p className="text-sm text-red-600">{unpaidInvoices.length} invoice(s) pending payment</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {invoices.map((inv) => {
                      const balance = parseFloat(inv.grand_total) - parseFloat(inv.amount_paid || 0);
                      return (
                        <tr key={inv.id}
          ...
          <div className="rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto"
            ...
            <form onSubmit={handlePayment} className="p-5 space-y-4">
              ...
              <div>
                ...
                <select className="input-field" value={payForm.invoice_id}
                  onChange={(e) => {
                    const inv = invoices.find(i => i.id === parseInt(e.target.value));
                    const bal = inv ? parseFloat(inv.grand_total) - parseFloat(inv.amount_paid || 0) : 0;
                    setPayForm(p => ({ ...p, invoice_id: e.target.value, amount: bal.toFixed(2) }));
                  }} required>
                  <option value="">-- Select Invoice --</option>
                  {invoices.filter(i => i.payment_status !== "paid").map(inv => {
                    const bal = parseFloat(inv.grand_total) - parseFloat(inv.amount_paid || 0);
                    return (
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 font-medium" style={{color:'var(--text-3)'}}>Rs.</span>
                  <input type="number" className="input-field pl-8" value={payForm.amount}
